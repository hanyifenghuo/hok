package com.hok.expection.core;
...
import com.hok.expection.tools.RecoverySilentSharedPrefsUtil;
import com.hok.expection.tools.RecoveryUtil;
...
public class RecoveryService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (mode == Recovery.SilentMode.RESTART) {
            ...
            restartAndClear();
        } else if (mode == Recovery.SilentMode.STOP_SELF) {
            stopSelf();
        }else {
            stopSelf();
        }
    ...
    private void recoverActivityStack(Intent o) {
        ...
        if (intents != null && !intents.isEmpty()) {
            ...
            if (!availableIntents.isEmpty()) {
                ...
                availableIntents.get(availableIntents.size() - 1).putExtra(RecoveryActivity.RECOVERY_MODE_ACTIVE, true);
